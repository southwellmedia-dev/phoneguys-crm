-- Update timeline functions to include customer comments

-- Drop and recreate get_ticket_timeline to include customer comments
...
BEGIN
  ...
  FROM (
    ...
    SELECT 
      ...
      'comment' as type,
      CASE 
        WHEN c.author_role = 'customer' THEN 'Customer: ' || c.content
        WHEN c.is_public = true THEN c.content
        ELSE 'Staff: ' || c.content
      END as description,
      NULL as status,
      (c.author_role = 'customer') as is_customer
    FROM comments c
    WHERE c.entity_type = 'ticket'
    AND c.entity_id = v_ticket_id
    AND (c.is_public = true OR c.author_role = 'customer')
  ) event;
...
-- Drop and recreate get_appointment_timeline to include customer comments
