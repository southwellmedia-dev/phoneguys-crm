            ...
            <Card className="overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-3">
                  ...
                  <span className="text-foreground">Created from Appointment</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {appointmentData.notes && (
                  <div>
                    ...
                    <p className="text-sm bg-slate-50 dark:bg-slate-800/50 p-3 rounded-md border border-slate-200 dark:border-slate-700">{appointmentData.notes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <div className="p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                          {order.device.device_type && (
                            <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border">
                              ...
                              <p className="text-sm font-semibold">{order.device.device_type}</p>
                            </div>
                          )}
                          {matchingCustomerDevice?.color && (
                            <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border">
                              <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">Color</p>
                              <p className="text-sm font-semibold">{matchingCustomerDevice.color}</p>
                            </div>
                          )}
                          {matchingCustomerDevice?.storage_size && (
                            <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border">
                              <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">Storage</p>
                              <p className="text-sm font-semibold">{matchingCustomerDevice.storage_size}</p>
                            </div>
                          )}
                          {order.device.release_year && (
                            <div className="bg-white dark:bg-slate-800 rounded-lg p-2.5 border">
                              <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">Year</p>
                              <p className="text-sm font-semibold">{order.device.release_year}</p>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex gap-3">
                        {(order.imei || matchingCustomerDevice?.imei) && (
                          <div className="flex-1 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-200 dark:border-slate-700 relative group">
                            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1 font-semibold">IMEI</p>
                            <div className="flex items-center justify-between">
                              <p className="text-sm font-mono">{order.imei || matchingCustomerDevice?.imei}</p>
                              <Button
                                variant="ghost"
                                size="sm"
                                ...
                                onClick={() => copyToClipboard(order.imei || matchingCustomerDevice?.imei || '', 'IMEI')}
                              >
                        ...
                        {(order.serial_number || matchingCustomerDevice?.serial_number) && (
                          <div className="flex-1 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-200 dark:border-slate-700 relative group">
                            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1 font-semibold">Serial Number</p>
                            <div className="flex items-center justify-between">
                              <p className="text-sm font-mono">{order.serial_number || matchingCustomerDevice?.serial_number}</p>
                              <Button
                                variant="ghost"
                                size="sm"
                                className="opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6 p-0 text-muted-foreground hover:bg-slate-100 dark:hover:bg-slate-800"
                                onClick={() => copyToClipboard(order.serial_number || matchingCustomerDevice?.serial_number || '', 'Serial Number')}
                              >
                                <Copy className="h-3 w-3" />
                              </Button>
                            </div>
                      ...
                      {order.device.parts_availability && (
                        <div className={cn(
                          ...
                          <div className={cn(
                            ...
                            order.device.parts_availability === 'available' ? "bg-green-500" :
                            order.device.parts_availability === 'limited' ? "bg-amber-500" : "bg-red-500"
                          )} />
                ...
                {!order.device && (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-foreground mb-2">No Device Information</h3>
                    <p className="text-sm text-muted-foreground mb-4 max-w-sm mx-auto">
                      This ticket doesn't have device information associated with it yet. 
                      Edit the ticket to add device details.
                    </p>
                    <Button variant="outline" asChild>
                      <Link href={`/orders/${orderId}/edit`} className="inline-flex items-center gap-2">
                ...
                {/* Action Button */}
                {!matchingCustomerDevice && order.customer_id && order.device_id && (
              ...
              <CardHeader className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                <CardTitle className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary" className="ml-auto">
                    {order.ticket_services.length} item{order.ticket_services.length > 1 ? 's' : ''}
                  </Badge>
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                <div className="space-y-3">
                  {order.ticket_services.map((ts: any, index: number) => (
                    <div key={ts.id} className="group relative p-4 bg-slate-50 dark:bg-slate-800/50 rounded-md border border-slate-200 dark:border-slate-700 hover:shadow-sm transition-all">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-semibold text-base mb-1.5">{ts.service?.name}</h4>
                            
                            <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground mb-2">
                              {ts.service?.category && (
                                <span className="capitalize">{ts.service.category.replace('_', ' ')}</span>
                              )}
                            ...
                            {ts.technician_notes && (
                              <div className="mt-2 p-2 bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-800/50 rounded text-sm">
                                <span className="font-medium text-amber-700 dark:text-amber-400">Note:</span> {ts.technician_notes}
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  <div className="pt-3 mt-3 border-t flex justify-between items-center">
                    <div className="text-sm text-muted-foreground">
                      <span>{order.ticket_services.length} service{order.ticket_services.length > 1 ? 's' : ''}</span>
                      {order.ticket_services.some((ts: any) => ts.service?.estimated_minutes) && (
                        <span className="ml-3">
                          • Est. {order.ticket_services.reduce((sum: number, ts: any) => 
                    ...
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground uppercase tracking-wider mb-0.5">Total Amount</p>
          ...
          <Card className="overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <CardContent className="pt-4">
              <div className="space-y-4">
                {/* Issues Section */}
                <div>
                  <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Reported Issues</h4>
                  {order.repair_issues && order.repair_issues.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {order.repair_issues.map((issue: string, index: number) => (
                        <div key={index} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-md text-sm font-medium border border-slate-200 dark:border-slate-700">
                ...
                {/* Description Section */}
                <div>
                  <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Problem Description</h4>
                  {order.description ? (
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/30 rounded-lg border">
                      <p className="text-sm leading-relaxed whitespace-pre-wrap">{order.description}</p>
                    </div>
                  ) : (
                    <div className="text-sm text-muted-foreground italic p-3 bg-slate-50 dark:bg-slate-900/30 rounded-lg border border-dashed">
                ...
                {order.priority && (
                  <div className="flex items-center justify-between p-3 bg-amber-50 dark:bg-amber-950/20 rounded-lg border border-amber-200/50 dark:border-amber-800/50">
                    <span className="text-sm font-medium text-amber-700 dark:text-amber-400">Priority Level</span>
              ...
              <div className="flex items-center justify-between">
                ...
                {order.ticket_notes?.length > 0 && (
                  <Badge variant="secondary">
                    {order.ticket_notes.length} note{order.ticket_notes.length > 1 ? 's' : ''}
                  </Badge>
                )}
              </div>
            </CardHeader>
            <CardContent className="pt-4">
              {order.ticket_notes?.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={note.note_type === 'internal' ? 'default' : 'outline'} className="text-xs">
                            {note.note_type}
                          </Badge>
                        </div>
                      ...
                      <p className="text-sm leading-relaxed mt-3">{note.content}</p>
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-xs mt-1">Notes and comments will appear here</p>
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <CardTitle className="text-sm font-semibold flex items-center justify-between">
                ...
                {order.customers?.id && (
                  <Link href={`/customers/${order.customers.id}`}>
                    <Button size="sm" variant="ghost" className="h-7 px-2 text-xs hover:bg-slate-200 dark:hover:bg-slate-700">
                      View →
                    </Button>
                  </Link>
                )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1 mt-1">
                      ...
                      >
                        ...
                        <span className="truncate">{order.customers?.email || "No email"}</span>
                      </a>
                      <a 
                ...
                <div className="space-y-4">
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 pb-4">
                      <p className="text-xs font-medium text-primary">Created</p>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 pb-4">
                      ...
                      <p className="text-sm font-semibold">{new Date(order.updated_at).toLocaleDateString()}</p>
                      <p className="text-xs text-muted-foreground">{new Date(order.updated_at).toLocaleTimeString()}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    <div className="relative z-10">
                      <div className={cn(
                        ...
                        order.status === 'in_progress' ? "bg-blue-100 dark:bg-blue-900/30" :
                        order.status === 'cancelled' ? "bg-red-100 dark:bg-red-900/30" :
                        order.status === 'on_hold' ? "bg-yellow-100 dark:bg-yellow-900/30" :
                        ...
                        <div className={cn(
                          ...
                          order.status === 'on_hold' ? "bg-yellow-500" :
                          "bg-slate-500"
                        )} />
                      </div>
                    </div>
